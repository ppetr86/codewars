...
public class ComputeRanksDuplicate {
    ...
    public static int[] computeRanks(int number, int[][] games) {
        ...
        if (games.length == 0) {
            ...
            return Arrays.stream(result).map(each -> each + 1).toArray();
        }
        List<TeamStats> statsList = createTeams(number);

        for (int i = 0; i < games.length; i++) {
            int[] currentGame = games[i];
            int firstTeam = currentGame[0];
            int secondTeam = currentGame[1];
            int scoredByFirst = currentGame[2];
            int scoredBySecond = currentGame[3];
            addScoreForGame(statsList, currentGame, firstTeam, secondTeam, scoredByFirst, scoredBySecond);
        }
        calculateTeamBalance(statsList);
    ...
    private static void compareTeamResulsAndOrderTeams(List<TeamStats> statsList) {
        Collections.sort(statsList, new Comparator<TeamStats>() {
            @Override
    ...
    private static void calculateTeamBalance(List<TeamStats> statsList) {
        statsList.forEach(each -> each.setBalance(each.getForScore() - each.getAgainstScore()));
    }
    ...
    public static class TeamStats {
        ...
        public TeamStats(int teamName, int forScore, int againstScore, int points) {
            this.teamName = teamName;
